public final class Projectile extends Renderable {
	...
	static final void drawEntities(int var0, int var1, int var2, int var3) {
		...
		if (BuddyRankComparator.localPlayer.x >> 7 == Client.destinationX
				&& BuddyRankComparator.localPlayer.y >> 7 == Client.destinationY) {
		...
		for (GraphicsObject var4 = (GraphicsObject) Client.graphicsObjects
				.last(); var4 != null; var4 = (GraphicsObject) Client.graphicsObjects.previous()) {
		...
		if (!Client.isCameraLocked) {
			...
			if (WorldMapSectionType.clientPreferences.getRoofsHidden()) {
				...
				var14 = WorldMapDecorationType.getTileHeight(class381.cameraX, class471.cameraZ,
						TaskHandler.Client_plane);
				if (var14 - class351.cameraY < 800
						&& (Tiles.Tiles_renderFlags[TaskHandler.Client_plane][class381.cameraX >> 7][class471.cameraZ >> 7]
								& 4) != 0) {
					var13 = TaskHandler.Client_plane;
				} else {
					var13 = 3;
		...
		for (var18 = 0; var18 < 5; ++var18) {
			if (Client.cameraShaking[var18]) {
				var19 = (int) (Math.random() * (double) (Client.cameraShakeIntensity[var18] * 2 + 1)
						- (double) Client.cameraShakeIntensity[var18]
						+ Math.sin((double) Client.cameraShakeSpeed[var18] / 100.0D
								* (double) Client.cameraShakeCycle[var18])
								* (double) Client.cameraMoveIntensity[var18]);
				if (var18 == 0) {
		...
		class1.scene.draw(class381.cameraX, class351.cameraY, class471.cameraZ, class311.cameraPitch,
				class110.cameraYaw, var12);
		...
		for (var26 = 0; var26 < Client.overheadTextCount; ++var26) {
			...
			while (var31) {
				...
				for (int var32 = 0; var32 < var26; ++var32) {
					if (var28 + 2 > Client.overheadTextYs[var32] - Client.overheadTextAscents[var32]
							&& var28 - var30 < Client.overheadTextYs[var32] + 2
							&& var37 - var29 < Client.overheadTextXOffsets[var32] + Client.overheadTextXs[var32]
							&& var29 + var37 > Client.overheadTextXs[var32] - Client.overheadTextXOffsets[var32]
							&& Client.overheadTextYs[var32] - Client.overheadTextAscents[var32] < var28) {
			...
			if (Client.chatEffects == 0) {
				...
				if (Client.overheadTextEffects[var26] == 0) {
					class7.fontBold12.drawCentered(var38, var0 + Client.viewportTempX, Client.viewportTempY + var1,
							var33, 0);
				}

				if (Client.overheadTextEffects[var26] == 1) {
					class7.fontBold12.drawCenteredWave(var38, var0 + Client.viewportTempX, Client.viewportTempY + var1,
							var33, 0, Client.viewportDrawCount);
				}

				if (Client.overheadTextEffects[var26] == 2) {
					class7.fontBold12.drawCenteredWave2(var38, var0 + Client.viewportTempX, Client.viewportTempY + var1,
							var33, 0, Client.viewportDrawCount);
				}

				if (Client.overheadTextEffects[var26] == 3) {
					class7.fontBold12.drawCenteredShake(var38, var0 + Client.viewportTempX, Client.viewportTempY + var1,
							var33, 0, Client.viewportDrawCount, 150 - Client.overheadTextCyclesRemaining[var26]);
				}

				if (Client.overheadTextEffects[var26] == 4) {
					var34 = (150 - Client.overheadTextCyclesRemaining[var26])
							* (class7.fontBold12.stringWidth(var38) + 100) / 150;
					Rasterizer2D.Rasterizer2D_expandClip(var0 + Client.viewportTempX - 50, var1,
							var0 + Client.viewportTempX + 50, var3 + var1);
					class7.fontBold12.draw(var38, var0 + Client.viewportTempX + 50 - var34, Client.viewportTempY + var1,
							var33, 0);
					Rasterizer2D.Rasterizer2D_setClip(var0, var1, var0 + var2, var3 + var1);
				}
				...
				if (Client.overheadTextEffects[var26] == 5) {
					...
					Rasterizer2D.Rasterizer2D_expandClip(var0,
							Client.viewportTempY + var1 - class7.fontBold12.ascent - 1, var0 + var2,
							Client.viewportTempY + var1 + 5);
					class7.fontBold12.drawCentered(var38, var0 + Client.viewportTempX,
							var35 + Client.viewportTempY + var1, var33, 0);
					Rasterizer2D.Rasterizer2D_setClip(var0, var1, var0 + var2, var3 + var1);
				}
			} else {
				class7.fontBold12.drawCentered(var38, var0 + Client.viewportTempX, Client.viewportTempY + var1,
		...
		if (Client.isLoading) {
			byte var40 = 0;
			var22 = var40 + NetCache.NetCache_pendingPriorityWritesCount
