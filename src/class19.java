...
public class class19 implements Callable {
	...
	static void playJingle(int var0, int var1) {
		if (WorldMapSectionType.clientPreferences.getCurrentMusicVolume() != 0 && var0 != -1) {
			class163.method853(class36.field149, var0, 0, WorldMapSectionType.clientPreferences.getCurrentMusicVolume(),
					false);
		...
		while (true) {
			...
			if (var1.bitsRemaining(Client.packetWriter.serverPacketLength) >= var2 + 12) {
				...
				if (var4 != var3 - 1) {
					...
					var6.method561(BuddyRankComparator.localPlayer.pathX[0] + var8,
							BuddyRankComparator.localPlayer.pathY[0] + var9, var7 == 1);
					continue;
				}
			}
